...
<head>
	...
	$(document).ready(function(){
		...
		$("#kdhtable tr").click(function(){
			...
			$("#ssNo").val(ssNo);
			$("#tForm").attr('action','/eportfolio/aaa.ssm')
			.submit();
...
<body>
	...
	SmemberVO svo = eftlist.get(0); 
					String grade = svo.getSrMembervo().getSrGrade();
					String class_= svo.getSrMembervo().getSrClass();%>
				...
				<tbody>
					...
					%>	
						<tr data-num=<%=svo.getSsNo() %>>
							...
							<td><%=svo.getSsName() %></td>
							<td><%=svo.getSrMembervo().getSrGrade()%></td>
							<td><%=svo.getSrMembervo().getSrClass()%></td>
							<td><%=svo.getSrMembervo().getSrNum()%></td>
